...
HRESULT CProject::FindProjectID(CDataBase db, ULONG nSimulationId, ULONG &nProjectID)
{
	if (!db) throw db;
	nProjectID = 0;
...
HRESULT CProject::LoadFromConsole(CDataBase db, ULONG nSimulationId)
{
	if (!db) throw db;
	CDataBase::SELECT sel;
...
HRESULT CProject::LoadFromVisualisation(CDataBase db, ULONG nProjectID)
{
	if (!db) throw db;
...
HRESULT CProject::Store(CDataBase db)
{
	if (!db) throw db;
...
HRESULT CProject::CleanUp(CDataBase db, ULONG nSimulationId)
{
	if (!db) throw db;
...
HRESULT CProject::CleanUpAll(CDataBase db)
{
	if (!db) throw db;
...
HRESULT CProject::DropTables(CDataBase db)
{
	if (!db) throw db;
