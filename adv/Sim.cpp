...
HRESULT CSim::LoadFromConsole(CDataBase db, ULONG nSimulationID)
{
	...
	try
	{
		...
		if (!sel) return Log(ERROR_PROJECT);
		...
		else return Log(ERROR_UNRECOGNISED_STRING, L"dispatcher algorithm", strAlgorithm.c_str());
...
HRESULT CSim::Store(CDataBase db, ULONG nSimulationID)
{
	if (!db) return db;
	if (!GetBuilding())
		return Log(ERROR_INTERNAL, L"Project stored without the building set.");
	try
	{
		...
		if (GetBuilding())
		{
			HRESULT h = GetBuilding()->Store(db, m_nProjectID);
			if (FAILED(h)) return h;
		}
...
HRESULT CSim::Update(CDataBase db, AVLONG nTime)
{
	...
	{
		if (m_nProjectID == 0)
			return Log(ERROR_INTERNAL, L"Project update run with ID=0");

		// Store the Journeys
		AVULONG nLifts = GetLiftCount();
		for (AVULONG i = 0; i < nLifts; i++)
		{
			HRESULT h = GetLift(i)->Store(db, m_nProjectID);
			if (FAILED(h)) return h;
		}
		...
		{
			HRESULT h = GetPassenger(i)->Store(db, m_nProjectID);
			if (FAILED(h)) return h;
		}
