...
public:
	...
	DWORD Load(CBuildingSrv *pBuilding, dbtools::CDataBase db, ULONG nSimulationId);	// returns S_OK if successful
	DWORD Load(CBuildingSrv *pBuilding, LPCOLESTR pName);		// returns S_OK if successful
	void Print();
...
public:
	...
	void Run(CBuildingSrv::LIFT *pLIFT, CSimLoader &loader, AVULONG nLiftId);
