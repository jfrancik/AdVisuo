...
public:
	...
	CLftGroup *GetLftGroup(int i)				{ return m_groups[i]; }		// get lift group by index
	CLftGroup *FindLftGroup(int i);											// find lift group by id
	std::vector<CLftGroup*> &GetLiftGroups()	{ return m_groups; }		// get lift group collection

	CLftGroup *AddLftGroup();
	void DeleteLftGroups();
...
protected:
	virtual CLftGroup *CreateLftGroup(AVULONG iIndex) = 0;
};
