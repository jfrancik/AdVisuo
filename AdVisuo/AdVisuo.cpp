...
BOOL CAdVisuoApp::InitInstance()
{
	...
	try
	{
		   ...
		   (cmdInfo.m_strFileName.Left(8).Compare(L"advisuo:") == 0 || cmdInfo.m_strFileName.Left(5).Compare(L"http:") == 0))
		{
			...
			static CDlgSplash dlg;
			dlg.DoNonModal();

			//dlg.GetDlgItem(IDC_TEXT)->SetWindowText(L"dupa");
			m_pOutList = (CListBox*)(dlg.GetDlgItem(IDC_DEBUG));
			...
			DWORD t = ::GetTickCount(); while (::GetTickCount() < t+750) ;
...
void CAdVisuoApp::OnFileDownload()
{
	...
	{
		AfxMessageBox(pe.ErrorMessage().c_str());
		return ;
	}
	catch (_com_error ce)
	{
