...
protected:
	...
	struct ACTION_SUBS_EX : public ACTION_SUBS
	{
		...
		LIST_OF_SUBS *pColl;
		typename LIST_OF_SUBS::iterator iter;
...
public:
	...
	virtual HRESULT __stdcall UnSubscribe(FWULONG nTimeStamp, ACTION_SUBS *pSubs)
	{
		...
		{
			ACTION_EVENT event = { NULL, NULL, nTimeStamp, EVENT_UNSUBSCRIBE, 0, NULL, pSubs };
	...
	virtual HRESULT __stdcall Suspend(FWULONG nTimeStamp)
	{
		...
		{
			// just suspended
			ACTION_EVENT event = { NULL, NULL, nTimeStamp, EVENT_SUSPENDED, 0, NULL, NULL };
			SendEventEx(&event);
	...
	virtual HRESULT __stdcall Resume(FWULONG nTimeStamp)
	{
		...
		if (m_nSuspended == 0)
		{
			...
			ACTION_EVENT event = { NULL, NULL, nTimeStamp, EVENT_RESUMED, 0, NULL, NULL };
			SendEventEx(&event);
		}
		return S_OK;
	}
...
//				return m_pCurEvent->pSender->ApplyEnvelope(pEvent, timeIn, timeRef, pfOut);
	...
	{
		ACTION_EVENT event = { NULL, NULL, nTimeStamp, nEvent, nSubCode, pPtr, NULL };
...
public:
	...
	{
		//pEvent->pSender = this;
		if (!pEvent->pOriginator) pEvent->pOriginator = this;
		...
		for (list<ACTION_SUBS_EX*>::iterator i = lstCalls.begin(); i != lstCalls.end(); i++)
		{
			...
			if (pSubs->pSubscriber->IsSuspended() == S_OK)
				continue;
			if (pSubs->pSubscriber == pEvent->pSender)
				continue;
			if ((pSubs->nFlags & ACTION_MASK_MODE) == ACTION_MORITURI)
				continue;
			...
			if ((pSubs->nFlags & ACTION_MASK_MODE) != ACTION_ONCE && (pSubs->nFlags & ACTION_RESERVED_1) == 0)
			{
				...
				pSubs->pSubscriber->RaiseEvent(pEvent->nTimeStamp, EVENT_BEGIN, pSubs->pSubscriber->StartTime(), (IFWUnknown*)pEvent);
			}
			...
			if (pSubs->pSubscriber->SendEventEx(&myEvent) == S_OK)
			...
			if (pSubs->pSubscriber && pSubs->pSubscriber->IsMorituri(&myEvent) == S_OK)
			{
				if ((pSubs->nFlags & ACTION_MASK_MODE) != ACTION_ONCE)		// no EVENT_END for ACTION_ONCE actions
					pSubs->pSubscriber->RaiseEvent(myEvent.nTimeStamp, EVENT_END, pSubs->pSubscriber->CompleteTime(), (IFWUnknown*)pEvent);
				UnSubscribe(pEvent->nTimeStamp, pSubs);
			}
		}
...
typedef CTAction<FWUNKNOWN<IAction, IID_IAction, IAction> > ACTION;
