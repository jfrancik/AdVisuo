...
{
	IKineNode *pBone = GetSim()->GetBuilding()->GetLiftNode(GetId());
	ITransform *p = NULL;
...
void CLift::AnimateDoor(AVULONG nShaft, AVULONG nStorey, bool bOpen, AVULONG timeStart, AVULONG timeDuration)
{
	...
	IKineNode *pDoorRI = GetSim()->GetBuilding()->GetRDoor(GetId());
	IKineNode *pDoorLX = GetSim()->GetBuilding()->GetExtLDoor(nShaft, nStorey);
	IKineNode *pDoorRX = GetSim()->GetBuilding()->GetExtRDoor(nShaft, nStorey);
